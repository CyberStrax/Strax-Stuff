...
- Update the Ansible `hosts` file by using `sudo /etc/ansible/hosts`. Information added will include the Web Server VM IP addresses and the ELK VM IP address, along with instruction to use Python 3. The playbooks utilized in this deploymeny identify the host as either `webservers` or `elk`. The headers used in the `hosts` file correspond with the host names used in the playbooks in order to ensure services are installed to the proper IP address. The following is a sample of the Ansible `hosts` file:
```
